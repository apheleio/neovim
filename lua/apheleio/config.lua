...
---@alias Palette { pure: string, focus: string, clarity: string, harmony: string, bliss: string, whisper: string, whimsy: string, dim: string, calm: string, serene: string, ethereal: string }
---@alias PaletteColor "pure" | "focus" | "clarity" | "harmony" | "bliss" | "whisper" | "whimsy" | "dim" | "calm" | "serene" | "ethereal"
...
config.options = {
	...
	---defaulting to "main" for light and "dark" for dark.
	---@type "auto" | Variant
	variant = "auto",
...
local function migrate(options)
	if options.bold_vert_split then
		local border = options.groups.border or "dim"
